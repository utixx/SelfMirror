...
class UNet3D(nn.Module):
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=64, stage=4, layer_order='cr', num_groups=8,
                 **kwargs):
        super(UNet3D, self).__init__()

        if isinstance(f_maps, int):
            # use 4 levels in the encoder path as suggested in the paper
            f_maps = create_feature_maps(f_maps, number_of_fmaps=stage)  # f_maps * 2(**0, 2, 4, 8)  8->bottm
        ...
        for i in range(len(reversed_f_maps) - 1):
            ...
            decoder_sider = SingleConv(out_feature_num, out_channels, kernel_size=1)
            decoder_siders.append(decoder_sider)

        self.decoders = nn.ModuleList(decoders)
        self.decoder_siders = nn.ModuleList(decoder_siders)
        self.side_activation = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        decoder_side_outs = []
        for decoder_sider, decoder_out in zip(self.decoder_siders, decoder_outputs):
            side_out = decoder_sider(decoder_out)
            # side_out = self.side_activation(side_out)   # bceloss
            decoder_side_outs.append(side_out)
