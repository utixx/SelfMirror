...
class trainset(Dataset):
    ...
    def __init__(self, name_list, coordinate_list, noise_img_all, stack_index):
        ...
        self.stack_index = stack_index    # list
...
def get_gap_z(args, img, stack_num):
    whole_x = img.shape[2]
    whole_y = img.shape[1]
    whole_t = img.shape[0]
    ...
    # print('h_num -----> ',h_num)
    # print('s_num -----> ',s_num)
    gap_z = math.floor((whole_t - args.patch_t * 2) / (s_num - 1))
    # print('gap_z -----> ',gap_z)
    return gap_z
...
def train_preprocess_lessMemoryMulStacks(args):
    ...
    for im_name in list(os.walk(im_folder, topdown=False))[-1][-1]:
        ...
        gap_z2 = get_gap_z(args, noise_im, stack_num)
        args.gap_z = gap_z2
        # print('gap_z2 -----> ',gap_z2)
        ...
        # print('int((whole_t-patch_z2+gap_z2)/gap_z2) -----> ',int((whole_t-patch_z2+gap_z2)/gap_z2))
        for x in range(0, int((whole_y - patch_y + gap_y) / gap_y)):
            for y in range(0, int((whole_x - patch_x + gap_x) / gap_x)):
                for z in range(0, int((whole_t - patch_z2 + gap_z2) / gap_z2)):
...
def test_preprocess_lessMemoryNoTail_chooseOne(args, N):
    patch_y = args.patch_y
    patch_x = args.patch_x
    patch_z2 = args.patch_t
    gap_y = args.gap_y
    gap_x = args.gap_x
    gap_z2 = args.gap_z
    cut_w = (patch_x - gap_x) / 2
    cut_h = (patch_y - gap_y) / 2
    cut_s = (patch_z2 - gap_z2) / 2
    im_folder = args.datasets_path + '//' + args.datasets_folder
    ...
    num_s = math.ceil((whole_t - patch_z2 + gap_z2) / gap_z2)
    # print('int((whole_y-patch_y+gap_y)/gap_y) -----> ',int((whole_y-patch_y+gap_y)/gap_y))
    # print('int((whole_x-patch_x+gap_x)/gap_x) -----> ',int((whole_x-patch_x+gap_x)/gap_x))
    # print('int((whole_t-patch_z2+gap_z2)/gap_z2) -----> ',int((whole_t-patch_z2+gap_z2)/gap_z2))
    for x in range(0, num_h):
        for y in range(0, num_w):
            for z in range(0, num_s):
                ...
                if z == 0:
                    single_coordinate['stack_start_s'] = z * gap_z2
                    single_coordinate['stack_end_s'] = z * gap_z2 + patch_z2 - cut_s
                    single_coordinate['patch_start_s'] = 0
                    single_coordinate['patch_end_s'] = patch_z2 - cut_s
                elif z == num_s - 1:
                    single_coordinate['stack_start_s'] = whole_t - patch_z2 + cut_s
                    single_coordinate['stack_end_s'] = whole_t
                    single_coordinate['patch_start_s'] = cut_s
                    single_coordinate['patch_end_s'] = patch_z2
                else:
                    single_coordinate['stack_start_s'] = z * gap_z2 + cut_s
                    single_coordinate['stack_end_s'] = z * gap_z2 + patch_z2 - cut_s
                    single_coordinate['patch_start_s'] = cut_s
                    single_coordinate['patch_end_s'] = patch_z2 - cut_s
...
def test_preprocess_chooseOne(args, img_id):
    ...
    patch_y = args.patch_y
    patch_x = args.patch_x
    patch_z2 = args.patch_z
    gap_y = args.gap_y
    gap_x = args.gap_x
    gap_z2 = int(args.patch_z * (1 - args.overlap_factor))
    cut_w = (patch_x - gap_x) / 2
    cut_h = (patch_y - gap_y) / 2
    cut_s = (patch_z2 - gap_z2) / 2
